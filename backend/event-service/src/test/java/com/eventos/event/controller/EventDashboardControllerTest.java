...
public class EventDashboardControllerTest {
        ...
        void testGetDashboardMetrics() {
                ...
                org.springframework.security.core.context.SecurityContext context = mock(
                ...
                try {
                        when(eventRepository.countByTenantIdAndStartDateAfter(eq(tenantId), any(LocalDateTime.class)))
                                        .thenReturn(1L);
                        when(eventRepository.findTop5ByTenantIdAndStartDateAfterOrderByStartDateAsc(eq(tenantId),
                        ...
                        when(eventTimelineItemRepository.findAllByTenantIdAndCompletedOrderByScheduledTimeAsc(tenantId,
                                        false))
                                        .thenReturn(Collections.singletonList(mockTask));
                        when(eventRepository.findAllByIdInAndTenantId(anyList(), eq(tenantId)))
                                        .thenReturn(Collections.singletonList(mockEvent));

                        when(invoiceRepository.countOutstandingByTenantId(tenantId)).thenReturn(2L);
                        when(invoiceRepository.sumOutstandingAmountByTenantId(tenantId))
                                        .thenReturn(BigDecimal.valueOf(15000));
                        when(vendorContractRepository.findAllByTenantId(tenantId)).thenReturn(Collections.emptyList());
