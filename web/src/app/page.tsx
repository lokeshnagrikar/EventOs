...
export default function Home() {
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row bg-background text-foreground transition-colors duration-200">
      ...
      <header className="sticky top-0 z-30 flex items-center justify-between px-6 py-4 border-b border-border bg-card/85 backdrop-blur-md md:hidden">
      ...
      `}>
        <div>
          ...
          <button 
            ...
            className="w-full flex items-center justify-between gap-2 px-3 py-2 border border-border rounded-md bg-background text-sm text-zinc-400 hover:bg-secondary focus-visible:ring-2 focus-visible:ring-primary focus-visible:outline-none transition-all mb-6"
          >
          ...
          <nav className="space-y-1">
            <a 
              href="/" 
              onClick={() => setIsMobileMenuOpen(false)}
              className="flex items-center gap-3 px-3 py-2 rounded-md bg-secondary text-primary font-medium text-sm focus-visible:ring-2 focus-visible:ring-primary focus-visible:outline-none transition-all"
            >
            ...
            <a 
              href="/crm" 
              onClick={() => setIsMobileMenuOpen(false)}
              className="flex items-center gap-3 px-3 py-2 rounded-md text-zinc-400 hover:text-foreground hover:bg-secondary/50 focus-visible:ring-2 focus-visible:ring-primary focus-visible:outline-none text-sm transition-all"
            >
              <Users size={18} />
              CRM / Leads
            </a>
        ...
        <header className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-6 border-b border-border">
          ...
          <div className="flex items-center gap-3">
            <button 
              ...
              className="px-3 py-2 border border-border bg-card hover:bg-secondary disabled:opacity-50 focus-visible:ring-2 focus-visible:ring-primary focus-visible:outline-none rounded-md text-xs font-semibold transition-all"
            >
              {isSyncing ? "Syncing..." : "Sync Database"}
            </button>
            <button 
              onClick={() => (window.location.href = "/crm")}
              className="flex items-center gap-2 px-4 py-2 bg-primary text-primary-foreground focus-visible:ring-2 focus-visible:ring-primary focus-visible:outline-none rounded-md text-sm font-medium hover:opacity-90 transition-all shadow-sm"
            >
              <Plus size={16} />
              Create New Lead
            </button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          {showWidget("totalLeads") && (
            <ErrorBoundary title="Total Leads">
              <div className="p-6 rounded-xl border border-border bg-card shadow-sm flex flex-col justify-between min-h-[140px]">
          ...
          {showWidget("revenueMetrics") && (
            <ErrorBoundary title="Revenue Stream">
              <div className="p-6 rounded-xl border border-border bg-card shadow-sm flex flex-col justify-between min-h-[140px] md:col-span-1">
                {isLoading ? renderShimmer() : (
                  <>
                    <div className="flex items-center justify-between mb-2">
          ...
          {showWidget("upcomingEvents") && (
            <ErrorBoundary title="Upcoming Schedules">
              <div className="p-6 rounded-xl border border-border bg-card shadow-sm md:col-span-2 space-y-4">
                ...
                {isLoading ? (
                  ...
                  <div className="space-y-3 text-xs">
                    {dashboard?.pendingPayments?.map((p, idx) => (
                      <div key={idx} className="flex justify-between items-center p-3 border border-border rounded-lg bg-background/50">
                        <div>
                ...
                {isLoading ? (
                  ...
                  <div className="space-y-3 text-xs">
                    {dashboard?.teamTasks?.map((task) => (
                      <div key={task.id} className="flex items-start gap-2.5 p-3 border border-border rounded-lg bg-background/50">
                        <input 
                          type="checkbox" 
                          id={`task-${task.id}`}
                  ...
                  <div className="relative border-l border-border pl-4 space-y-6 text-xs ml-2 py-1">
                    {dashboard?.recentActivity?.map((activity) => (
                      <div key={activity.id} className="relative">
                        <div className="absolute -left-[21px] mt-1 h-2 w-2 rounded-full bg-primary ring-4 ring-card"></div>
