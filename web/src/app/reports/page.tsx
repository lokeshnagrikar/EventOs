...
export default function ReportsPage() {
  ...
  const actorName = user?.firstName ? `${user.firstName} ${user.lastName || ""}`.trim() : (user?.email?.split("@")[0] || "Administrator");
  const agencyName = memberships[0]?.companyName || "EventOS Workspace";
        ...
        <div className="hidden print:block text-zinc-900 bg-white p-10 font-sans space-y-8 absolute top-0 left-0 w-full min-h-screen">
          <div className="flex justify-between items-center border-b pb-4">
            <div>
              ...
              <p className="text-xs text-zinc-500">Date: {new Date().toLocaleDateString()} • Generated By: {actorName}</p>
            </div>
          ...
          <div className="pt-20 flex justify-between text-xs">
            <div className="border-t border-zinc-300 w-48 text-center pt-2">
              ...
              <p className="text-[10px] text-zinc-400">{actorName}</p>
            </div>
            <div className="border-t border-zinc-300 w-48 text-center pt-2">
